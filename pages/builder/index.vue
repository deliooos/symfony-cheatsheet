...
const { data } = await useAsyncData('builder', () => queryContent('/builder').find())
...
<template>
  <main class="classic-main">
    <h1 class="text-5xl font-bold mb-3">SFCS Builder</h1>
    <form class="mb-10">
      <label class="label">
        <span class="label-text">Recherche</span>
      </label>
      <div class="input-group">
        <input v-model="search" type="text" placeholder="Rechercher un builder..." class="input input-bordered" />
        <button class="btn btn-square" disabled>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
        </button>
      </div>
    </form>
    <h5 class="font-bold text-2xl">Construction en cours</h5>
